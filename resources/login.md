...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| `sn` | number | 序列号[\[1\]](#login-sn) 实验性 |
| `platform` | string?[\[3\]](#login-nullable) | 平台名称 |
| `user` | [User](./user.md)?[\[3\]](#login-nullable) | 用户对象[\[2\]](#login-user) |
| `status` | [LoginStatus](#def-login-status) | 登录状态 |
| `adapter` | string | [适配器名称](../advanced/internal.md#platform-vs-adapter) 实验性 |
| `features` | string\[]? | [平台特性](../protocol/api.md#platform-features)列表 实验性 |
