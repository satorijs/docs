...
当前 Bot 的组合 ID，格式为 `{platform}:{selfId}`。 -->
...
当前 Bot 最近一次遇到的错误信息。在 `bot.offline()` 时设置，在 `bot.online()` 时清除。 -->

<!-- ### bot.callbacks

- 类型: `Dict<Function>`

按钮交互的回调函数映射。键为按钮 ID，值为回调函数。 -->
...
- **name:** `string` API 方法名
- **session:** `Partial<Session>` 会话实例
- 返回值: `Promise<boolean>`

检测 Bot 是否支持特定的 API 方法。
...
检测 Bot 是否具有特定权限。当 `name` 以 `bot.` 开头时，将委托给 `bot.supports()` 进行检测。

### bot.toJSON()

- 返回值: [`Login`](../resources/login.md#def-login)
